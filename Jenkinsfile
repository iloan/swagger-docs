        ...
        stage('Wiz IAC scan') {
            steps {
                script { 
                    ...
                    sh "./wizcli iac scan --path . --tag repo=iloan-$GIT_REPO_NAME --tag branch=$GIT_BRANCH --tag commit=$GIT_COMMIT  -p custom-policy-secrets --policy-hits-only  "
                    sh "./wizcli iac scan --path . --tag repo=iloan-$GIT_REPO_NAME --tag branch=$GIT_BRANCH --tag commit=$GIT_COMMIT  -p custom-policy-high-critical-severity-misconfigurations --policy-hits-only  "
                }
            } 
        }
    }
